...
window.addEventListener('load', function () {
    ...
    function PutDelayedBid() {
        function MyBid() {
            ...
            if (MinBidSum <= MaxBidId.value) {
                console.log('MyBid: ' + document.getElementById("vi-cdown_timeLeft").innerText + ' / ' + Date().toLocaleString());
                bidBtn.click();
            }
        }
        /* Timer function */
        ...
        console.log('PutDelayedBid: ' + document.getElementById("vi-cdown_timeLeft").innerText + ' / ' + Date().toLocaleString());
        document.getElementById("MyBidBtn").disabled = true;
        let myVar = setInterval(checkTime, 500);
    }
    ...
    /* Place bid button */
    function OnBidBtn() {
        console.log("OnBidBtn: " + document.getElementById("vi-cdown_timeLeft").innerText + ' / ' + Date().toLocaleString());
        BidTimestamp = Date.now();
        checkElement('#confirm_button')
            .then((element) => {
            const confirmBtn = document.getElementById("confirm_button");
            console.log('Confirm: ' + document.getElementById("vi-cdown_timeLeft").innerText + ' / ' + Date().toLocaleString() + ', ' + (Date.now() - BidTimestamp) + ' ms.');

            checkElement('span.ui-component-icon__CONFIRMATION')
                .then((element) => {
                console.log('Confirm: Successful at ' + (Date.now() - BidTimestamp) + ' ms.');
            });
            //confirmBtn.click();
        });
    }
    ...
    function OnMaxBidId() {
        ...
        document.getElementById("MaxBidId").value = Number(document.querySelector('div.bidAmt').innerText.match(/\$([0-9]+[\.,0-9]*)/)[1]);
    }
